...
const repos = require('./repositories').REPOSITORIES
const {
  isDirectMessage,
  isBotMessage,
  isMessage,
...
const {time_elapsed_string, add_jira_link_to_title} = require("./format");
...
module.exports = function server () {
  ...
  function buildMessage (data) {
    ...
    const prMessages = includedPrs
      .sort((a, b) => {
        return a.created_at.localeCompare(b.created_at);
      })
      .map((pr) => `${time_elapsed_string(Date.parse(pr.created_at), ' ago')} ${add_jira_link_to_title(pr.title)} - <${pr.html_url}|Review PR>`)
